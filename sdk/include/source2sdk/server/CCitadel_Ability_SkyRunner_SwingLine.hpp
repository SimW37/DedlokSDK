...
namespace source2sdk::server
{
    ...
    #pragma pack(push, 1)
    class CCitadel_Ability_SkyRunner_SwingLine : public server::CCitadelBaseAbility
    {
    public:
        // metadata: MNetworkEnable
        client::ESwingState_t m_eSwingState; // 0xba0        
        [[maybe_unused]] std::uint8_t pad_0xba1[0x3]; // 0xba1
        // metadata: MNetworkEnable
        entity2::GameTime_t m_SwingStartTime; // 0xba4        
        // metadata: MNetworkEnable
        entity2::GameTime_t m_SwingEndTime; // 0xba8        
        // metadata: MNetworkEnable
        Vector m_vecSwingPoint; // 0xbac        
        Vector m_vecCurrentPosition; // 0xbb8        
        // metadata: MNetworkEnable
        float m_flIdealSpringLength; // 0xbc4        
        [[maybe_unused]] std::uint8_t pad_0xbc8[0xc8];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_SkyRunner_SwingLine because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_SkyRunner_SwingLine) == 0xc90);
};
