...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            bool m_bSecondBomb; // 0x858            
            uint8_t _pad0859[0x3]; // 0x859
            std::int32_t m_nBeepSoundBuildupCount; // 0x85c            
            float m_flBeepSoundIntervalBias; // 0x860            
            float m_flBeepSoundMaxFrequency; // 0x864            
            float m_flArmingDuration; // 0x868            
            ...
            char m_vecBeepIntervals[0x18]; // 0x870            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CCitadel_Projectile_BloodBomb because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CCitadel_Projectile_BloodBomb) == 0x888);
    };
};
