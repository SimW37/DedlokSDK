...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_VacuumAuraModifier[0x10]; // 0x15f0            
            // metadata: MPropertyStartGroup "+Vacuum Properties"
            float m_flAirSpeedMax; // 0x1600            
            float m_flFallSpeedMax; // 0x1604            
            float m_flAirDrag; // 0x1608            
            float m_flMaxMovespeed; // 0x160c            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CAbilityVacuumVData because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CAbilityVacuumVData) == 0x1610);
    };
};
