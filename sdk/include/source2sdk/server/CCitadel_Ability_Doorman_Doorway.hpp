...
namespace source2sdk::server
{
    ...
    #pragma pack(push, 1)
    class CCitadel_Ability_Doorman_Doorway : public server::CCitadelBaseAbility
    {
    public:
        ...
        // CHandle<server::CCitadel_DoorwayPortal> m_hPendingDoor;
        char m_hPendingDoor[0x4]; // 0xbb8        
        // metadata: MNetworkEnable
        bool m_bHasPendingDoor; // 0xbbc        
        [[maybe_unused]] std::uint8_t pad_0xbbd[0x4b];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCitadel_Ability_Doorman_Doorway because it is not a standard-layout class
    static_assert(sizeof(CCitadel_Ability_Doorman_Doorway) == 0xc08);
};
