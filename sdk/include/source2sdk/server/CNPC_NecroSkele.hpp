...
namespace source2sdk
{
    namespace server
    {
        ...
        {
        public:
            uint8_t _pad1790[0x28]; // 0x1790
            // metadata: MNetworkEnable
            source2sdk::entity2::GameTime_t m_tSpawnTime; // 0x17b8            
            // metadata: MNetworkEnable
            VectorWS m_vecCastLocation; // 0x17bc            
            // metadata: MNetworkEnable
            bool m_bDontMove; // 0x17c8            
            uint8_t _pad17c9[0x3]; // 0x17c9
            // metadata: MNetworkEnable
            // metadata: MNotSaved
            float m_flAttackRange; // 0x17cc            
            // metadata: MNetworkEnable
            // metadata: MNotSaved
            float m_flSpawnDuration; // 0x17d0            
            uint8_t _pad17d4[0xc];
            
            // Datamap fields:
            // CHandle< CBaseEntity > m_hCastingAbility; // 0x17a4
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CNPC_NecroSkele because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CNPC_NecroSkele) == 0x17e0);
    };
};
