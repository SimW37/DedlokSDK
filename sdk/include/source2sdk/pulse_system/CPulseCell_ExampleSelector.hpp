...
namespace source2sdk::pulse_system
{
    ...
    // static metadata: MPulseSelectorAllowRequirementCriteria
    #pragma pack(push, 1)
    class CPulseCell_ExampleSelector : public pulse_runtime_lib::CPulseCell_BaseFlow
    {
    public:
        pulse_runtime_lib::PulseSelectorOutflowList_t m_OutflowList; // 0x48        
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CPulseCell_ExampleSelector because it is not a standard-layout class
    static_assert(sizeof(CPulseCell_ExampleSelector) == 0x60);
};
