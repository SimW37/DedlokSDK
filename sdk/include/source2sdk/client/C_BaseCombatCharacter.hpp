...
namespace source2sdk::client
{
    ...
    public:
        ...
        modellib::AttachmentHandle_t m_leftFootAttachment; // 0xed8        
        modellib::AttachmentHandle_t m_rightFootAttachment; // 0xed9        
        [[maybe_unused]] std::uint8_t pad_0xeda[0x2]; // 0xeda
        client::C_BaseCombatCharacter__WaterWakeMode_t m_nWaterWakeMode; // 0xedc        
        float m_flWaterWorldZ; // 0xee0        
        float m_flWaterNextTraceTime; // 0xee4        
        [[maybe_unused]] std::uint8_t pad_0xee8[0x60];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in C_BaseCombatCharacter because it is not a standard-layout class
    static_assert(sizeof(C_BaseCombatCharacter) == 0xf48);
};
